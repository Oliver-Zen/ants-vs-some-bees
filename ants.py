...
class Insect:
    """An Insect, the base class of Ant and Bee, has f and a Place."""
...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        if hasattr(self, 'damage') and not self.doubled:
            self.damage *= 2
            # print("DEBUG: ant {}'s damage is successfully doubled".format(self))
            self.doubled = True
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        def helper(place, index):
            if place.is_hive:
                return None
            elif place.bees and self.lower_bound <= index <= self.upper_bound:
                return random_bee(place.bees)
            else: # place.bees is None
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        else: # amount < self.health
            for bee in bees[:]:           # don't modify a list whiling iteraitng it
...
# The HungryAnt Class
class HungryAnt(Ant):
    """ HungryAnt eat Bee from its place """
    name = 'Hungry'
    food_cost = 4
    chewing_turns = 3
    implemented = True 
...
class ScubaThrower(ThrowerAnt):
    """ A subclass of ThrowerAnt that is more costly and waterproof, but otherwise identical to its base class. """
    food_cost = 6
    is_waterproof = True
    name = 'Scuba'
    implemented = True
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        """
        # BEGIN Problem 12
        print("DEBUG: QueenAnt is acting")
        def helper(place):
            # print("DEBUG: helping")
            if place:
                if place.ant:
                    # print("DEBUG: Now doubling the damage of {}".format(place.ant))
                    place.ant.double()
                    if place.ant.is_container and place.ant.ant_contained:
                        # print("DEBUG: Now doubling the damage of {}".format(place.ant.ant_contained))
                        place.ant.ant_contained.double()
                helper(place.exit)
        helper(self.place.exit)
        # print("DEBUG: doublement finished")
    ...
    def remove_from(self, place):
        # BEGIN Problem 12
        self.place = place  # to be determined
        # END Problem 12
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        # BEGIN Problem Optional 2
        def helper(place, distance, result):  # -> dict key: Insect, value: distance
            if not place.is_hive:             # stop adding if reach the HIVE
                # add all ANTs to the dictionary
                if place.ant:
                    if distance == 0:
                ...
                # add all BEEs to the dictionary
                if place.bees:
                    bees = place.bees
                    for bee in bees:
                        result[bee] = distance
                # go to next PLACE
                helper(place.entrance, distance + 1, result)
        result = {}
        helper(self.place, 0, result)
        return result
        # END Problem Optional 2

    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        return max(self.damage - self.insects_shot * 0.0625 - 0.25 * distance, 0)
        # END Problem Optional 2

    def action(self, gamestate):
        insects_and_distances = self.insects_in_front()
        # print("DEBUG: insects_and_distances: {}".format(insects_and_distances))
        for insect, distance in insects_and_distances.items():
            damage = self.calculate_damage(distance)
            # print("DEBUG: now damaging {0} by {1} health".format(insect, damage))
